...
// ── Colors pulled from the app's splash screen gradient ──────────────────────
const COLORS = {
  blueLight:   '#5BB8F5',
  blueMid:     '#2E86D4',
  blueDark:    '#1A4F9C',
  accent:      '#FFFFFF',       // white text/icons on blue
  text:        '#1A1A1A',
  subtext:     '#5A5A5A',
  background:  '#FFFFFF',
  star:        '#FFD700',
  border:      '#E8F0FB',       // blue-tinted border
  cardBg:      '#F4F8FF',       // blue-tinted card background
  pillInactive:'#DDE9F8',
  verified:    '#4CAF50',
};
...
function PortfolioCard({ uri, index }: { uri: string; index: number }) {
  return (
      <View style={styles.portfolioCard}>
        <Text style={styles.workTitle}>Work {index + 1}</Text>
  ...
  if (loading) {
    return (
        <LinearGradient colors={[COLORS.blueLight, COLORS.blueDark]} style={styles.centered}>
          ...
          <Text style={[styles.loadingText, { color: '#fff' }]}>Loading profile…</Text>
        </LinearGradient>
    );
  }
  ...
  if (error || !provider) {
    return (
        <LinearGradient colors={[COLORS.blueLight, COLORS.blueDark]} style={styles.centered}>
          ...
          <Text style={[styles.errorText, { color: '#fff' }]}>{error ?? 'Provider not found.'}</Text>
      ...
      <SafeAreaView style={styles.container}>
        ...
        <ScrollView ref={scrollRef} stickyHeaderIndices={[2]} showsVerticalScrollIndicator={false}>
            ...
            {provider.profile_image ? (
                <>
                  <Image
                      ...
                      resizeMode="cover"
                  />
                  {/* blue gradient overlay so it still feels on-brand */}
                  <LinearGradient
          ...
          <View style={styles.pillsContainer}>
            {(
                [
                  { key: 'about',     ref: aboutRef,      label: 'About' },
                  { key: 'portfolio', ref: portfolioRef,  label: 'Portfolio' },
                ] as const
            ).map(({ key, ref, label }) => (
                <TouchableOpacity
                    key={key}
          ...
          <View style={styles.contentPadding}>
            ...
            <View ref={aboutRef} style={styles.contentBlock}>
              ...
              {provider.tags.length > 0 && (
                  <>
                    <Text style={[styles.sectionTitle, { marginTop: 18 }]}>Specialisations</Text>
                    <View style={styles.tagsRow}>
                      {provider.tags.map((tag) => (
            ...
            <View ref={portfolioRef} style={styles.contentBlock}>
              ...
              {provider.portfolio_images.length > 0 ? (
                  provider.portfolio_images.map((uri, i) => (
                      <PortfolioCard key={i} uri={uri} index={i} />
                  ))
              ) : (
                  <EmptyCard icon="images-outline" text="No portfolio images uploaded yet." />
              )}
            </View>
